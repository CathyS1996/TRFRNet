...
def test():
    print('loading data......')
    test_data = getattr(datasets, opt.tdataset)(opt.root, opt.ttest_data_dir, mode='test')
    ...
    model = generate_model(opt,opt.model)
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            _ACC_overall, _IoU_poly, _IoU_bg, _IoU_mean = evaluate(torch.sigmoid(output), gt)
